...
class EncoderLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, n_layers=1):
        ...
        self.lstm = nn.LSTM(
            self.hidden_size,  # input size = hidden_size because embedding layer output is that size
            self.hidden_size,
            self.n_layers,
...
class DecoderLSTM(nn.Module):
    def __init__(self, hidden_size, output_size, dropout_rate, max_length, n_layers=1):
    ...
    def forward(self, input, prev_hidden, encoder_hidden):
        ...
        output = F.relu(output)
        att_vec = torch.zeros(self.MAX_SIZE)
        output, hidden = self.lstm(output, prev_hidden)
        output = self.softmax(self.out(output[0]))
        return output, hidden, att_vec

    def initHidden(self):
        return (torch.zeros(self.n_layers, 1, self.hidden_size, device=device),
                torch.zeros(self.n_layers, 1, self.hidden_size, device=device))

class AttentionUnitLSTM(nn.Module):
    def __init__(self, hidden_size, max_size):
        """
            Attention as introduced in the paper.
            Has 3 learnable parameters:
        ...
        """
        super(AttentionUnitLSTM, self).__init__()
        self.hidden_size = hidden_size
...
class DecoderAttentionLSTM(nn.Module):
    def __init__(self, hidden_size, output_size, dropout_rate, max_length, n_layers=1):
        ...
        self.attention_layer = AttentionUnitLSTM(self.hidden_size, self.MAX_SIZE)
        self.embedding = nn.Embedding(output_size, hidden_size)

        self.lstm = nn.LSTM(
            hidden_size * 2,  # input size = hidden_size because embedding layer output is that size
            hidden_size,
            n_layers,
            dropout=self.dropout_rate
        )
    ...
    def forward(self, input, prev_hidden, encoder_hidden):
        ...
        # Decoder needs to calculate the attention (Read the comments in the AttentionUnit class)
        att_vec, context_vec = self.attention_layer(prev_hidden[-1], encoder_hidden)

        # Combine the Context Vextor with the Output (of the embeddings)
        output = torch.cat((output[-1], context_vec.unsqueeze(0)), 1)

        output, new_hidden = self.lstm(output.unsqueeze(0), prev_hidden)
        # Calculate the hidden state of the Unit.
        # The paper then states that this hidden state is concatenated to the
        # context vector and this is used to predict the next output action (a_i)
        # The tutorial used a log_softmax (likely to avoid overflow)
